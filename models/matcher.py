...
class HungarianMatcherHOI(nn.Module):
    ...
    def __init__(self, cost_obj_class: float = 1, cost_verb_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, binary: bool = False, cost_binary: float = 1):
        super().__init__()
...
def build_matcher(args):
    if args.hoi:
        return HungarianMatcherHOI(cost_obj_class=args.set_cost_obj_class, cost_verb_class=args.set_cost_verb_class, cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou, binary=args.binary, cost_binary=args.set_cost_binary)
    else:

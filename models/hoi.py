...
class DETRHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False, binary=False):
        super().__init__()
    ...
    def forward(self, samples: NestedTensor):
        ...
        features, pos = self.backbone(samples) # (Nested, N, 128, H, W)

        src, mask = features[-1].decompose() # (N, C, H, W), (N, H, W)
        assert mask is not None
        hs_box, hs_verb = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])[:2] # (6, N, Q, C), (6, N, Q, C)

        outputs_obj_class  = self.obj_class_embed(hs_box) # (6, N, Q, 80)
        outputs_sub_coord  = self.sub_bbox_embed(hs_box).sigmoid() # (6, N, Q, 4)
        outputs_obj_coord  = self.obj_bbox_embed(hs_box).sigmoid() # (6, N, Q, 4)
        outputs_verb_class = self.verb_class_embed(hs_verb) # (6, N, Q, 117)
        
        out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
               'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1]}
        outputs_binary_class = None
        if self.binary:
            outputs_binary_class = self.binary_class_embed(hs_box)
            out['pred_binary_logits'] = outputs_binary_class[-1]
            
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, outputs_binary_class)
        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, outputs_binary_class=None):
        ...
        if not self.binary:
            ...
            return [{'pred_obj_logits': a, 'pred_verb_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'pred_binary_logits': e}
                    for a, b, c, d, e in zip(outputs_obj_class[:-1], outputs_verb_class[:-1],
                                          outputs_sub_coord[:-1], outputs_obj_coord[:-1], outputs_binary_class[:-1])]
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_binary_labels(self, outputs, targets, indices, num_interactions):
        assert 'pred_binary_logits' in outputs
...
class PostProcessHOI(nn.Module):
    ...
    def __init__(self, subject_category_id, binary=False, pnms=-1):
        super().__init__()
        ...
        for idx in range(len(obj_scores)):
            ...
            if self.pnms > 0:
                ...
                for i in range(vs.shape[1]):
                    ...
                    idx = torch.argsort(vs[:, i]).cpu().numpy().tolist()[::-1]
                    for j in idx:

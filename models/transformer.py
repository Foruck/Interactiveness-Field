...
class CascadedTransformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        decoder_layer_verb = TransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout, activation, normalize_before)
        decoder_verb_norm = nn.LayerNorm(d_model)
        self.verb_decoder = TransformerDecoder(decoder_layer_box, num_decoder_layers_verb, decoder_verb_norm, return_intermediate=return_intermediate_dec)
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        out_box  = self.box_decoder(tgt_box, memory, memory_key_padding_mask=mask, pos=pos_embed, query_pos=query_embed)
        out_box  = out_box.transpose(1, 2)
        
        verb_query = out_box[-1]
        verb_query = verb_query.permute(1, 0, 2)
        tgt_verb   = torch.zeros_like(verb_query)
        out_verb   = self.verb_decoder(tgt_verb, memory, memory_key_padding_mask=mask, pos=pos_embed, query_pos=verb_query)
        return out_box, out_verb.transpose(1, 2), memory.permute(1, 2, 0).view(bs, c, h, w)
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        return hs.transpose(1, 2), hs.transpose(1, 2), memory.permute(1, 2, 0).view(bs, c, h, w) # (6, N, Q, C)
    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:
            output = layer(output, memory, tgt_mask=tgt_mask,
                           ...
                           pos=pos, query_pos=query_pos) # (Q, N, C)
        ...
        if self.return_intermediate:
            return torch.stack(intermediate) # (6, Q, N, C)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos),
                                   ...
                                   key_padding_mask=memory_key_padding_mask)[0] # (Q, N, C)
...
def build_transformer(args):
    ...
    if args.hoi and args.cascaded:
        return CascadedTransformer(

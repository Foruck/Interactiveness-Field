...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, label_nms=-1):
        self.img_set = img_set
        self.img_folder = img_folder
        with open(anno_file, 'r') as f:
            self.annotations = json.load(f)
        if label_nms > 0 and img_set == 'val':
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            ...
            if len(sub_obj_pairs) == 0:
                ...
                target['binary_labels'] = (1 - target['verb_labels'][:, 57]).long()
        else:
            target['boxes'] = boxes
            target['labels'] = classes
            target['id'] = idx
...
def build(image_set, args):
    ...
    dataset = HICODetection(image_set, img_folder, anno_file, transforms=make_hico_transforms(image_set),
                            num_queries=args.num_queries, label_nms=args.label_nms)
    if image_set == 'val':
